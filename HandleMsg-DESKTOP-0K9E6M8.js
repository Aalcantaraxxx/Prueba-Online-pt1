...
module.exports = HandleMsg = async (aruga, message) => {
	...
	case 'stimg':
            if (quotedMsg && quotedMsg.type == 'sticker') {
                ...
                aruga.reply(from, `¡Convirtiendo el Sticker en imagen! Por favor, espere un momento...`, id)
                const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
                await aruga.sendFile(from, imageBase64, 'imgsticker.jpg', 'Gracias por usar Sticker Factory', id)
                .then(() => {
                    console.log(`Sticker to Image Processed for ${processTime(t, moment())} Seconds`)
                })
        } else if (!quotedMsg) return aruga.reply(from, `Ups! Ocurrió un Error, Selecciona el Sticker que quieras convertir a Imagen. Si tu problema persiste contactate con soporte técnico https://www.instagram.com/sticker._.factory/?hl=es-la`, id)
        break
