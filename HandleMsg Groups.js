...
module.exports = HandleMsg = async (aruga, message) => {
    ...
    if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
        ...
        case 'kick':
            ...
            if (mentionedJidList.length === 0) return aruga.reply(from, 'Falló, agregue el bot como administrador de grupo.', id)
            if (mentionedJidList[0] === botNumber) return await aruga.reply(from, 'Lo sentimos, el formato del mensaje es incorrecto. \nNo puede emitir la cuenta del bot usted mismo', id)
            await aruga.sendTextWithMentions(from, `Solicitud recibida, problema:\n${mentionedJidList.map(x => `@${x.replace('@c.us', '')}`).join('\n')}`)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if (groupAdmins.includes(mentionedJidList[i])) return await aruga.sendText(from, 'Falló, no puede eliminar el administrador del grupo.')
		...
		case 'mutegrup':
            ...
            if (args[0] == 'on') {
				...
				aruga.reply(from, `Para cambiar la configuración del chat grupal para que solo los administradores puedan chatear\n\nusar:\n${prefix}mutegrup on --habilitar\n${prefix}mutegrup off --desactivar`, id)
			}
			break
		case 'setprofile':
			if (!isGroupMsg) return aruga.reply(from, 'Lo sentimos, ¡este comando solo se puede usar dentro de grupos!', id)
